<?php 
  ...
  $query_RecAssistant = "SELECT * FROM `member` WHERE `m_manager`='".$row_RecMember["m_sn"]."'";
  ...
  if(isset($_GET["action"])&&($_GET["action"]=="delete")){  
    ...
    //重新導向回到主畫面
    header("Location: member_assistant.php");
  }
  ...
  if(isset($_POST["action"])&&($_POST["action"]=="join")){
    ...
    if (mysqli_num_rows($RecFindUser)>0){
      header("Location: member_assistant.php?errMsg=1");
    }else{
      ...
      header("Location: member_assistant.php?loginStats=1");
...
<!DOCTYPE html>
  ...
  <body>
    ...
    <script language="javascript">
      alert('此帳號已使用過。\n');
      window.location.href='member_assistant.php';
    </script>
    <?php } ?>
