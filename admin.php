<?php 
  ...
  /*
  $t=time(); 
  $TimeOut = "<script type='text/javascript' language='javascript'>
                ...
                var t = ".((int)$_SESSION['session_time'] - $t).";
                function showTime(){    
                  t -= 1;    
                  var m = parseInt(t/60);  
                  if(t<=0) location.href='logout.php';  
                  document.getElementById('div1').innerHTML= '".$myTime." | ' + t+'s('+m+'m) Auto Logout.';   
  ...
  if(isset($_POST["action"])&&($_POST["action"]=="update")){
    ...
    if (md5($_POST["pwd"]) != $row_RecMember["m_pwd"]){
      ...
      if ($_POST["m_pwd"] == '' && $_POST["m_passwordrecheck"] == '') {
        $query_insert = "UPDATE `member` SET `m_pwd` = '".md5($_POST["pwd"])."', `m_phone`= '".$_POST["m_phone"]."', `m_email` = '".$_POST["m_email"]."', `h_id` = '".$_POST["h_id"]."' WHERE `m_sn` = '".$row_RecMember["m_sn"]."' ";
        mysqli_query($conn, $query_insert);
        $updateHospital = "UPDATE `member` SET `h_id`='".$_POST["h_id"]."' WHERE `m_manager`='".$row_RecMember["m_sn"]."'";
        mysqli_query($conn, $updateHospital);
        header("Location: admin.php?loginStats=1");
      }else{
        $query_insert = "UPDATE `member` SET `m_pwd` = '".md5($_POST["m_pwd"])."', `m_phone`= '".$_POST["m_phone"]."', `m_email` = '".$_POST["m_email"]."', `h_id` = '".$_POST["h_id"]."' WHERE `m_sn` = '".$row_RecMember["m_sn"]."' ";
        mysqli_query($conn, $query_insert);
        $updateHospital = "UPDATE `member` SET `h_id`='".$_POST["h_id"]."' WHERE `m_manager`='".$row_RecMember["m_sn"]."'";
        mysqli_query($conn, $updateHospital);
        header("Location: admin.php?loginStats=1");
